...
void *epoll_init(struct event_loop *eventLoop) {
    epoll_dispatcher_data *epollDispatcherData = (epoll_dispatcher_data *)malloc(sizeof(epoll_dispatcher_data));
    ...
    epollDispatcherData->events = (struct epoll_event *)calloc(MAXEVENTS, sizeof(struct epoll_event));
